...
- (BOOL)mkdir
{
	if (![[NSFileManager defaultManager] fileExistsAtPath:self]) {
		BOOL yesORno = [[NSFileManager defaultManager] createDirectoryAtPath:self withIntermediateDirectories:YES attributes:nil error:nil];
		return yesORno;
	}
	return NO;
}
...
- (NSDictionary*)parseURLParameters
{
	if(self.length==0)
		return nil;
	NSString * tail = [[self componentsSeparatedByString:@"?"] lastObject];
	if(tail.length==0)
		return nil;
